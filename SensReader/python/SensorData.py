...
class RGBDFrame:
    ...
    def decompress_color_jpeg(self):
        return imageio.imread(self.color_data, format='jpeg')
...
class SensorData:
    ...
    def export_depth_images(self, output_path, image_size=None, frame_skip=1):
        ...
        for f in range(0, len(self.frames), frame_skip):
            ...
            if image_size is not None:
                depth = cv2.resize(depth, (image_size[1], image_size[0]), interpolation=cv2.INTER_NEAREST)
            with open(os.path.join(output_path, f"{f}.png"), 'wb') as file:  # write 16-bit
                writer = png.Writer(width=depth.shape[1], height=depth.shape[0], bitdepth=16)
                depth = depth.tolist()
    ...
    def export_poses(self, output_path, frame_skip=1):
        ...
        print('Exporting', len(self.frames) // frame_skip, 'camera poses to', output_path)
        for f in range(0, len(self.frames), frame_skip):
            self.save_mat_to_file(self.frames[f].camera_to_world, os.path.join(output_path, f"{f}.txt"))
